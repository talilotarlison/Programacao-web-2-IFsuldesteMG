...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$nome || !$datan || !$email || !$senha || !$confirma) {
        ...
        $stmt = $db->prepare("INSERT INTO usuarios (nome, datan, email, senha) VALUES (?, ?, ?, ?)");
        try {
            $stmt->execute([$nome, $datan, $email, $senhaHash]);
...
<body>
    ...
    <form method="post" action=<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>>
        ...
        <button type="button" onclick="window.location.href='index.php'" style="margin-top:10px; background:#ccc; color:#333;">Voltar para Login</button>
    </form>
</body>
</html>